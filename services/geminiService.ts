...
const analysisSchema = {
  ...
  required: ["summary", "complexityScore", "swot", "redFlags", "negotiationPoints"]
};
...
export const analyzeDocument = async (documentText: string, language: Language): Promise<AnalysisResult> => {
  ...
  const prompt = `
    ...
    1.  A concise summary of the document's core purpose.
    2.  A legal complexity score on a scale of 1 to 10, where 1 is extremely simple and 10 is exceptionally complex.
    3.  A SWOT analysis (Strengths, Weaknesses, Opportunities, Threats) from the perspective of the primary party receiving the document.
    4.  A list of potential 'red flags'. For each, provide a clear title for the flag and explain the associated risk.
    5.  A list of specific 'Points to Negotiate'. These should be actionable suggestions for improving the terms. For each point, provide a clear title (the 'point') and an 'explanation' of what to ask for and why it's beneficial.
